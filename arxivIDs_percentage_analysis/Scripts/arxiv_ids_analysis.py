...
ax.set_title(
    "Percentage of arXiv id references throughout the years\n papers were taken from the hep-ex category"
)
plt.tight_layout()
...
with open(
    "arxivIDs_percentage_analysis/arxiv_id_percentage_grqc.json", "r"
) as datafile:
...
with open(
    "arxivIDs_percentage_analysis/arxiv_id_percentage_hep-ex.json", "r"
) as datafile:
    data = json.load(datafile)
